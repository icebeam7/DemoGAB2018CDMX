...
[assembly: Dependency(typeof(ServicioAudioRecorder))]
namespace DemoGAB2018CDMX.Droid.Servicios
{
    public class ServicioAudioRecorder : IServicioAudioRecorder
    {
        int RECORDER_BPP = 16;
        ...
        {
            var path = System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal);
            return Path.Combine(path, "audio.wav");
        }

        string GetTextFilename()
        {
            var path = System.Environment.GetFolderPath(System.Environment.SpecialFolder.Personal);
            return Path.Combine(path, "texto.wav");
        }
        ...
        public string WriteAudioDataToFile(byte[] data)
        {
            var filename = GetTextFilename();
            using (var outputStream = new FileOutputStream(filename))
            {
                outputStream.Write(data);
            }
            return filename;
